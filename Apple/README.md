...
1. **Локальный деплой в Solar2D Simulator (убирает предупреждения)**:
   ...
   ./deployLocal.sh
   ```
   Скрипт автоматически соберёт нативную библиотеку `http3.dylib`, упакует платформенные архивы `data.tgz` и зарегистрирует плагин в `~/Solar2DPlugins/com.solar2d.http3/plugin.http3/`.
