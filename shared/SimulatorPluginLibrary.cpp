...
static unsigned long __stdcall RaceAndRequestThreadFunc(void* param) {
    ...
    if (race->quicSuccess == 0 && race->quicProgress == 0 && !race->winnerAssigned) {
        ...
        // СОЕДИНЕНИЕ ПОДНЯТО — ЗАПАСНОЙ ПУТЬ БОЛЬШЕ НЕ НУЖЕН.
        //
        // Здесь было второе окно: ждали ОТВЕТА по установленному соединению и,
        // не дождавшись, поднимали запрос ещё и по TCP. Задумывалось это как
        // страховка от соединения, которое встало молча уже после подключения.
        //
        // Никакой порог тут не работает. Сначала стояли жёсткие 800 мс, и на них
        // дублировался каждый длинный опрос игры. Порог сделали долей таймаута
        // (60%) — игровой опрос перестал дублироваться, а голосовой продолжил:
        // сервер держит его ответ около 10.5 с при таймауте 11 с, то есть 95%
        // отпущенного времени, и ЛЮБОЙ порог внутри таймаута срабатывает раньше
        // ответа. Время попросту не отличает «сервер намеренно держит» от
        // «соединение умерло»; эту разницу знает только вызывающий.
        //
        // Цена ошибки была не только в трафике: на каждый настоящий опрос
        // сервер держал ещё один десятисекундный запрос, уже никому не нужный.
        //
        // Поэтому окна больше нет. Молчащее соединение теперь ограничено
        // таймаутом запроса — он с недавних пор честно доходит до нативного
        // слоя, — а дальше Lua-обёртка повторит запрос через network.request.
        // Ровно так же ведёт себя network.request в Solar2D, где такой гонки
        // нет вовсе.
        LogMsg("RaceAndRequestThreadFunc: QUIC соединился, запасной путь не нужен");
    } else {
